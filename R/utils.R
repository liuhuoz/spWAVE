...
generate_meta_coord <- function(
  ...
  km_coord <- 
    cbind.data.frame(
      km_result$centers,
      cluster=paste0("clu_",rownames(km_result$centers))
    )
  ...
  km_coord <- coord %>%
    ...
    dplyr::right_join(km_coord,by="cluster") %>%
    dplyr::select(x,y,cluster,spot_id) %>%
    as.data.frame()
  return(list(meta=km_coord,single_coords=coord))
}
...
#' Generated by \code{\link{generate_meta_coord}}.
...
generate_holed_coord <- function(
  ...
  coords_mat <- coords[,c("x","y")] %>% as.matrix()

  dist_temp <- spa_vectorized_pdist(coords_mat,coords_mat[center,,drop=FALSE])
  coords$dist <- dist_temp[,1]
  coords$position <- ifelse(coords$dist <= radius, "inner", "outer")

  inner_idx <- which(coords$position=="inner")
  holed_clu <- coords$cluster[inner_idx] %>%
    table() %>% names()

  inner_part <- coords[inner_idx,]
  inner_part$uni_id <- inner_part$barcode
  inner_part <- inner_part[,c("x","y","uni_id")]

  outer_idx <- which(!(meta_coords$cluster %in% holed_clu))
  outer_part <- meta_coords[outer_idx,]
  outer_part$uni_id <- outer_part$cluster
  outer_part <- outer_part[,c("x","y","uni_id")]
