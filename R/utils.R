...
#' Slice Triangular Sparse Matrix
#'
#' Slice Triangular Sparse Matrix by given row and col
#'
#' @param SpMat triangular sparse matrix
#' @param row rows, can be rownames or indices. If null, return all rows.
#' @param col cols, can be colnames or indices. If null, return all cols.
#' 
#' @importClassesFrom Matrix dgCMatrix CsparseMatrix
#' 
#' @details The goal of this function is to slice a large pairwise distance matrix 
#' storing in sparse for saving memory and time.
#' 
#' @export 
#' @return sliced dense sub-matrix.
#' @examples
#' #Generate lower triangular sparse matrix
#' mat1 <- matrix(runif(25)*10,5,5)
#' rownames(mat1) <- paste0("row_",1:5)
#' colnames(mat1) <- paste0("col_",1:5)
#' tri_SpMat <- as(mat1*lower.tri(mat1),"dgCMatrix")
#' #sub matrix by index
#' sub_mat1 <- slice_tri_SpMat(tri_SpMat,row=1:3,col=2:4)
#' #sub matrix by character
#' sub_mat2 <- slice_tri_SpMat(tri_SpMat,row=paste0("row_",1:3),col=paste0("col_",2:4))
#' #sub matrix by mixed args
#' sub_mat3 <- slice_tri_SpMat(tri_SpMat,row=paste0("row_",1:3),col=2:4)
#' #sub_mat1,2,3 are definitely equal.
slice_tri_SpMat <- function(SpMat,row=NULL,col=NULL){
  if(is.null(row)){row <- seq_len(nrow(SpMat))}
  if(is.null(col)){col <- seq_len(nrow(SpMat))}

  if(is.character(row)){
    row_idx <- which(rownames(SpMat) %in% row)
  }else{row_idx <- row}
  if(is.character(col)){
    col_idx <- which(colnames(SpMat) %in% col)
  }else{col_idx <- col}

  left_upper_mat <- SpMat[row_idx,col_idx,drop=FALSE] %>% as.matrix()
  right_lower_mat <- SpMat[col_idx,row_idx,drop=FALSE] %>% as.matrix()
  sub_mat <- left_upper_mat + t(right_lower_mat)

  #is.large <- (length(row_idx) <= (nrow(SpMat)/2)) || (length(col_idx) <= (ncol(SpMat)/2))
  #if(is.large){
    
  #}else{
    # sub_mat %<>% as.matrix()
    # sub_mat <- sub_mat+t(sub_mat)
    # update_row_idx <- seq_len(length(row_idx))
    # update_col_idx <- seq_len(length(col_idx))+length(row_idx)
    # sub_mat <- sub_mat[update_row_idx,update_col_idx,drop=FALSE]
  #}
...
#'
#' @param coords data.frame,spatial coordinates, 
...
#' @export 
generate_meta_coords <- function(
  coords,
  n_clu=nrow(coords)/10,
  nstart=42
){
  km_result <- stats::kmeans(coords[,c("x","y")], centers=n_clu, nstart = nstart)
  km_coords <- 
    cbind.data.frame(
      km_result$centers,
      cluster=as.integer(rownames(km_result$centers))
    )
  coords$cluster <- km_result$cluster

  km_coords <- coords %>%
    dplyr::group_by(cluster) %>%
    dplyr::summarise(
      spot_id = paste(barcode, collapse = ",")
    ) %>% 
    dplyr::right_join(km_coords,by="cluster") %>%
    dplyr::select(x,y,cluster,spot_id) %>%
    as.data.frame()
  return(list(meta=km_coords,single_coords=coords))
}
...
#'
#' @param meta_coords_list The meta coordinates list. 
#' Generated by \code{\link{generate_meta_coords}}.
#' @param center the barcode of the center spot. 
#' @param radius radius of the circle. Default is 500 (micrometer).
#' @param dist_SpMat a precalculated distance matrix in Sparse Matrix format.
#'
#' @import dplyr
#' 
...
generate_holed_coord <- function(
  ...
  coords <-  meta_coords_list[[2]]

  dist_temp <- slice_tri_SpMat(dist_SpMat,col=center)
  coords$dist <- dist_temp[,1]
  coords$position <- ifelse(coords$dist <= radius, "inner", "outer")

  holed_clu <- coords %>% 
    dplyr::filter(position=="inner") %>% 
    dplyr::pull(cluster) %>% 
    table() %>% names()
  ...
  inner_part <- 
    coords %>% 
    dplyr::filter(position=="inner") %>% 
    dplyr::mutate(uni_id=paste0("spot_",barcode)) %>% 
    select(x,y,uni_id)

  outer_part <- 
    meta_coords %>% 
    dplyr::filter(!(cluster %in% holed_clu)) %>% 
    dplyr::mutate(uni_id=paste0("clu_",cluster)) %>%
    select(x,y,uni_id)
